...
firebase_admin.initialize_app(cred, {"projectId": "pintox-app"})
...
def check_time_validity(
    start_time: DatetimeWithNanoseconds, end_time: DatetimeWithNanoseconds
):
    ...
    if not (
        appointment_start_time <= str_start_time < str_end_time <= appointment_end_time
    ):
        return False, "invalid time"
    docs = (
        appointments_ref.where(
            "startTime",
            ">",
    ...
    if docs is not None:
        for each_doc in docs:
            ...
            if (
                each_patient_start_time < start_time < each_patient_end_time
                or each_patient_start_time < end_time < each_patient_end_time
            ):
                return False, "unavailable time slot"
            if (
                each_patient_start_time > start_time
...
def check_date_validity(date: DatetimeWithNanoseconds):
    ...
    docs = (
        ...
        .where(
            ...
            "<",
            DatetimeWithNanoseconds(
                date.year, date.month, date.day + 1, tzinfo=datetime.timezone.utc
            ),
        )
        .get()
    )
...
def web_hooks(request):
    ...
    if dialogflow_request.get_intent_displayName() == "Create Appointment - User":
        ...
        dialogflow_request.get_intent_displayName() == "Create Appointment - Date - yes"
    ):
        ...
        if check_date_validity(intend_date):
            ...
            dialogflow_response.add(
                OutputContexts(
                    "pintox-app", session_id, "Create Appointment - Time", 200, {}
                )
            )
            ...
            dialogflow_response.add(
                OutputContexts(
                    "pintox-app", session_id, "Create Appointment - Date", 200, {}
                )
            )
        ...
        dialogflow_request.get_intent_displayName() == "Create Appointment - Time - yes"
    ):
        docu_dict = temp_ref.document(session_id).get().to_dict()
    ...
    response = app.response_class(
        response=dialogflow_response.get_final_response(), mimetype="application/json"
    )
    return response
